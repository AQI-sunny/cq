...
function showPeriodicHint() {
    const hints = [
        "1、耳机遗失帖里有一个人的性格特征很明显哦~尝试搜索相关名词或形容词？搜电梯也可以~（1/44）",
        "2、手套丢失帖里的关键词为四个字的名词哦~是一件物品~（2/44）",
        "3、嘿 你不会还没发现返回键吧？尝试在论坛搜索“搜索” ？（3/44）",
        "4、关于老赵两口子的旧新闻需要先找出更早的某则新闻，通过报刊名联想~（4/44）",
        "5、点击返回键有惊喜~别忘了搜一搜公寓所在地~六个字的关键词（5/44）",
        "6、不要忘了搜一搜记者名字~（6/44）",
        "7、某条路上有一个咖啡店~帅哥帖子里暗示了咖啡店名~转换大写繁体哦（7/44）",
        "8、两个公园都搜过了吗~不要只在论坛里搜索哦~（8/44）",
        "9、某个植物不要单独搜索哦~要和地名联合起来搜索~（9/44）",
        "10、市区报道里面有张图片很重要哦~（10/44）",
        "11、咖啡店首页出现的咖啡也搜一搜？（11/44）",
        "12、宝子有没有解锁某个工具呀？试试在论坛也搜索一下呢？（12/44）",
        "13、王大妈要去放什么灯呢？某个节日发生了什么呢？在哪一年呢？（13/44）",
        "14、嘿~如果发现有隐藏帖子但是看不到，有可能需要登入其他账户哦~可以想想谁的账户比较重要？（14/44）",
        "15、超市和咖啡店后台用户名都是全名拼音哦~（15/44）",
        "16、浩子的真名你可知晓啦？他发布的帖子里有提到一个...网站，去search一下？（16/44）",
        "17、鸡蛋帖子有透露老板姓氏哦~还有评论区曾提到xx姐的糖糕（17/44）",
        "18、xx县某个帖子存在？年前、快？年的字眼，可以穷举出一个新闻哦~记得要联系地名~（18/44）",
        "19、浩子母亲的出院日期需要穷举哦~联系许愿帖子和第一篇文章末尾可得知大概范围~英文试着全选一下第二篇文章？（19/44）",
        "20、旧论坛名字是电影清单第二列字竖着下来哦~（20/44）",
        "21、邀请码是薄荷英文和评论区提到的电影上映日期哦（21/44）",
        "22、超市后台密码和女儿小名、生日有关哦~可通过某工具进行推理（22/44）",
        "23、有早报就有。。？知道新闻内容后联想一下~父母一般会发布什么启事呢？（23/44）",
        "24、有一个已注销的用户发布的文章。。。这个文章好像是藏头哦。。（24/44）",
        "25、林猫电脑里的密钥似乎是通过凯撒＋维吉尼亚破解的呢（25/44）",
        "26、阿哲的电脑也连接一下？关于生日。。一般会买什么呢？去超市搜索一下相关订单？还有他会买什么药膏呢？联想火灾新闻（26/44）",
        "27、歌词注意看第二类~它们的歌名是什么呢？揪出尾巴可得知一个4字地点哦~（27/44）",
        "28、快递柜在末尾有暗示哦，几个人逃出来了呢？取件码和提到的日期有关哦~是几几年呢？（28/44）",
        "29、A门的密码就在躲弓箭的背景上哦（29/44）",
        "30、后面每个门的密码都在前一个门的档案里哦（30/44）",
        "31、B门的密码是一种花的英文哦~反复提到的~M开头的哦（31/44）",
        "32、C门的密码是两个字的拼音＋梅森数哦（32/44）",
        "33、D门的密码是蒹葭苍苍下一句的前两个字拼音哦（33/44）",
        "34、E门密码是某类鸟的外号英文＋书籍的影视化年份哦（34/44）",
        "35、实验室深处密码要通过档案号推理哦~猜猜是谁被关在里面呢？（35/44）",
        "36、有两个账户可登入哦~一个是林猫的...还有一个...保密！林猫的密码是宠物名字和生日哦，她的咖啡订单备注是一个字谜！（36/44）",
        "37、旧论坛有提到《xxx则》，解字谜可得五字书籍哦（37/44）",
        "38、小符咒游戏可以得到一个提取码哦~（38/44）",
        "39、提取到的某卷书~可解出一个四字家族名哦~Y开头X结尾~注意观察xx后裔~（39/44）",
        "40、连出天秤座星象得到一串密文，用base64解开素？（40/44）",
        "41、解出的4个字是8字书的前半部分哦，后半部分在守秘录里倒着揪尾巴~（41/44）",
        "42、8字书名缩写是YLCJMSZS，你猜对了吗？（42/44）",
        "43、神秘用户的用户名要使用栅栏密码哦~嫌麻烦也可以刷新大搜索页面截屏红色字体~（43/44）",
        "44、神秘用户的密码是一句话哦~支线末尾的英文单词合起来~顺序可以看看歌词~数字只有一个，在第二位哦~（44/44）"
    ];
...
function showMessage(message, type = 'info', duration = 3000) {
    ...
    // 设置较低的z-index，确保不影响其他弹窗
    msgDiv.style.zIndex = '1000'; // 降低优先级
    ...
    // 检查是否有其他弹窗正在显示 - 改进检测逻辑
    /* const checkPopupVisible = () => {
        const popups = [
            document.getElementById('dangerous-word-popup'),
    ...
    // 如果有重要弹窗在显示，则暂不显示坛子消息
   /*  if (checkPopupVisible()) {
        console.log('检测到重要弹窗显示，暂不显示坛子消息');
        return;
    } */

        // 检查是否有其他弹窗正在显示 - 修改为只检测特定弹窗
const checkPopupVisible = () => {
    // 只检测危险词弹窗和搜索警告弹窗，不检测阿哲弹窗
    const popups = [
        document.getElementById('dangerous-word-popup'),
        document.getElementById('search-alert')
        // 注意：这里不包含阿哲弹窗，因为阿哲弹窗时我们可能还想显示坛子消息
    ];
...
// 单独检查阿哲弹窗的函数
const isAzhePopupVisible = () => {
    const azhePopup = document.getElementById('azhe-popup');
    if (!azhePopup) return false;
    
    const style = window.getComputedStyle(azhePopup);
    return style.display === 'block' || 
           style.display === 'flex' || 
           azhePopup.style.display === 'block';
};

// 如果有重要弹窗在显示，则暂不显示坛子消息
if (checkPopupVisible()) {
    console.log('检测到危险词或搜索警告弹窗显示，暂不显示坛子消息');
    return;
}

// 如果是阿哲弹窗，可以显示消息（如果需要可以注释掉）
if (isAzhePopupVisible()) {
    console.log('阿哲弹窗显示中，但可以显示坛子消息');
    // 可以继续显示消息，或者添加特殊处理
    // 例如：调整消息位置避免重叠
    // msgDiv.style.bottom = 'calc(50% + 150px)'; // 放在弹窗下方
}
