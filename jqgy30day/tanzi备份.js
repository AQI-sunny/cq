        ...
        if (post.searchKeyword) {
            ...
            if (keywords.some(keyword => keyword === lowerQuery)) {
                foundPosts.push({
                    ...
                    date: post.date,
                    keyword: query,
                    postId: post.title + '_' + post.author // 创建唯一ID
                });
            }
        }
    });
...
function recordKeyword(keyword, isValid, foundCount = 0) {
    ...
    // 获取这个关键词当前已经找到的所有帖子
    const currentFoundPosts = keywordsData[keyword] ? keywordsData[keyword].foundPosts || 0 : 0;
    ...
    if (!keywordsData[keyword]) {
        keywordsData[keyword] = {
            ...
            firstFound: new Date().toISOString(),
            lastFound: new Date().toISOString(),
            foundPosts: foundCount, // 新搜索找到的帖子数
            // 新增：记录找到的具体帖子
            foundPostTitles: foundCount > 0 ? 
                findHiddenPostsInResults(keyword).map(post => post.title) : []
        };
    } else {
        keywordsData[keyword].count++;
        keywordsData[keyword].lastFound = new Date().toISOString();
        
        // 关键修复：只有当这次搜索找到的是新帖子时才增加计数
        if (foundCount > 0) {
            // 获取这次搜索找到的帖子标题
            ...
            // 找出新发现的帖子标题
            const newTitles = currentTitles.filter(title => 
                !existingTitles.includes(title)
            );
            
            // 更新帖子标题列表（去重）
            keywordsData[keyword].foundPostTitles = [
                ...new Set([...existingTitles, ...newTitles])
            ];
            
            // 只增加新发现的帖子数量
            keywordsData[keyword].foundPosts = currentFoundPosts + newTitles.length;
        }
    ...
    if (keywordsList) {
        if (keywords.length > 0) {
            ...
            keywordsList.innerHTML = keywords.map(([keyword, data]) => {
                ...
                // 可选：显示找到的帖子标题数量（调试用）
                const titleCount = data.foundPostTitles ? data.foundPostTitles.length : 0;
