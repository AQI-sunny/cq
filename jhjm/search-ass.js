...
(function () {
    ...
    class VisitTracker {
        ...
        navigateToPage(pageName) {
            ...
            // 构建目标 URL
            let targetUrl = baseUrl + pageName;
            ...
            tryFetch(targetUrl).then(success => {
                if (!success) {
                    ...
                    if (!pageName.endsWith('.html')) {
                        const targetWithHtml = targetUrl + '.html';
                        console.log('尝试添加 .html 后缀:', targetWithHtml);
                        tryFetch(targetWithHtml).then(htmlSuccess => {
                            if (!htmlSuccess) {
                                // 如果还是失败，尝试直接跳转（让服务器返回 404）
                                console.warn('页面可能不存在，尝试直接跳转');
